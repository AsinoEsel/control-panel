...
CHAR_MAP = {
    ...
    'E': 0x4f, 'F': 0x47, 'G': 0x5E, 'H': 0x37, 
    'I': 0x30, 'J': 0x3c, 'K': 0x57, 'L': 0x0e, 
    'M': 0x54, 'N': 0x76, 'O': 0x7e, 'P': 0x67, 
...
class Display:
    ...
    def __init__(self, spi: SoftSPI, ss: Pin, intensity=7):
        self.spi = spi
        self.ss = ss
    ...
    def decode_char(self, c):
        d = CHAR_MAP.get(c)
        return d if d != None else 0x00

    def write_to_buffer(self, s):
        l = len(s)
        if l < 8:
            s = s + ' ' * (8 - l)
        # print(f'writing "{s}" to buffer')
        for i in range(0,8):
            self._buffer[7 - i] = self.decode_char(s[i])
    ...
    def write_to_buffer_with_dots(self, s):
        ...
        while i < len_s:
            ...
            if x >= 8:
                ...
                self._buffer[7 - x] = self.decode_char(s[i]) | 0x80
                i += 1
            else:
                self._buffer[7 - x] = self.decode_char(s[i])

            x += 1
            i += 1

        while x < 8:
            self._buffer[7 - x] = self.decode_char(' ')
            x += 1

    def display(self):
        for i in range(0,8):
            self.set_register(REG_DIGIT_BASE + i, self._buffer[i])
