...
def generate_overloads(devices: Dict[str, Dict[str, str]]) -> str:
    ...
    for action_value_set, device_set in group_map.items():
        ...
        lines.append(
            f") -> Callable[[Callable[[TypedEvent[{base_value_type}]], None]], Callable[[TypedEvent[{base_value_type}]], None]]: ...")
        lines.append("")
    return "\n".join(lines)


def generate_callback_stub_file():
    header = '''from typing import Callable, List, Literal, Hashable, overload
from controlpanel.api.typed_event import TypedEvent
